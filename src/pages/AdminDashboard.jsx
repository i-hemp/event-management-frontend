...
const AdminDashboard = () => {
    ...
    // Booking Filter & Pagination
    const [filterEvent, setFilterEvent] = useState('ALL');
    const uniqueEvents = [...new Set(bookings.map(b => b.eventName).filter(Boolean))].sort();

    const filteredBookings = filterEvent === 'ALL'
        ? bookings
        : bookings.filter(b => b.eventName === filterEvent);
            ...
            <div className="card" style={{ marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <span style={{ fontSize: '0.875rem', color: '#666' }}>Showing {paginatedBookings.length} of {filteredBookings.length}</span>
                        <select
                            value={filterEvent}
